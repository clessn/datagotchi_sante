...
def select_batch(log_df, batch_info_dict):
    ...
    if batch_name == "new batch":
        ...
        users_selected = [
            uid
            for uid in log_df["user_id"].unique()
...
def show_users_approval(
    ...
    if to_check:
        ...
        st.write(
            "Users to check manually because they passed attention checks but failed completion code"
        )
...
def users_per_explanation(user_df, approved_user_ids):
    ...
    df_plot = pd.DataFrame(
        {"Condition": condition_counts.index, "Users": condition_counts.values}
    )
...
df_plot = pd.DataFrame({"Phase": phase_order, "Users": phase_counts.values})
...
if selected_user:
    st.write(
        f"User `{selected_user}` has finished phase: **{last_phase_dict[selected_user]}**"
    )
...
if batch_name != "new batch":
    ...
    (
        ...
        refused_returned,
        refused_rejected,
    ) = get_users_approval(logs, batch_df)
    show_users_approval(
        to_approve,
        to_reject,
        to_check,
