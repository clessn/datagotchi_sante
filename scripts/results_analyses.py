...
def approved_users(batch_info_dict):
    ...
    for batch_df in batch_info_dict.values():
        approved_users_set.update(
            batch_df[batch_df["Status"] == "APPROVED"]["Participant id"].unique()
        )
...
def write_filtered_db():
    ...
    experiment_approved_users_list = attention_check_users(
        logs_raw, prolific_approved_users_list
    )
...
def get_phase_logs(logs, phase_id, questions_ids):
    ...
    filtered_logs = logs[
        (logs["phase_id"] == phase_id) & (logs["question_id"].isin(questions_ids))
    ]
    # For each question_id, keep only the log with the most recent timestamp
    latest_logs_df = filtered_logs.loc[
        filtered_logs.groupby("question_id")["timestamp"].idxmax()
    ]
...
def get_essaim_score(logs, answers):
    ...
    for log in logs.itertuples():
        ...
        sum_answer_values += (
            answer_value - 1
        )  # we remove 1 because it is based on a 1-6 scale, we want a 0-5 scale
...
def clean_results():
    ...
    for user_id in unique_users:
        ...
        start_time = user_logs.loc[
            user_logs["log_type"] == "started", "timestamp"
        ].min()
        end_time = user_logs.loc[user_logs["log_type"] == "finished", "timestamp"].max()
        study_duration = end_time - start_time
        results_df.loc[results_df["user_id"] == user_id, "study_duration"] = (
            study_duration
        )
        ...
        knowledge_questions_ids = questions[
            (questions["group_id"] == "knowledge")
            & (questions["question_id"] != "knowledge_04")
        ]["question_id"].tolist()
        # get logs for knowledge before and after phases
        knowledge_before_logs = get_phase_logs(
            user_logs, "knowledge_before", knowledge_questions_ids
        )
        knowledge_after_logs = get_phase_logs(
            user_logs, "knowledge_after", knowledge_questions_ids
        )
        # compute scores
        ...
        # save scores
        results_df.loc[results_df["user_id"] == user_id, "knowledge_before_score"] = (
            knowledge_before_score
        )
        results_df.loc[results_df["user_id"] == user_id, "knowledge_after_score"] = (
        ...
        if explain_type == "explain_interactive":
            results_df.loc[results_df["user_id"] == user_id, "explain_retry"] = len(
                explain_logs
            )
        else:
        ...
        results_df.loc[
            results_df["user_id"] == user_id, "predicted_wellbeing_score"
        ...
        essaim_questions_ids = questions[(questions["group_id"] == "essaim")][
            "question_id"
        ].tolist()
        # get logs for knowledge before and after phases
        ...
        # save scores
        results_df.loc[results_df["user_id"] == user_id, "observed_wellbeing_score"] = (
            essaim_score
        )
        ...
        manipulation_logs = get_phase_logs(
            user_logs, "satisfaction", manipulation_question_ids
        )
        ...
        satisfaction_questions_ids = questions[
        ...
        sociodemo_questions_ids = questions[questions["group_id"] == "sociodemo"][
            "question_id"
        ].tolist()
        ...
        for log in sociodemo_logs.itertuples():
            ...
            if log.question_id == "sociodemo_09":
                answer_content = int(
                    answers[answers["answer_id"] == answer_id]["answer_weight"].iloc[0]
                )
            else:
                answer_content = answers[answers["answer_id"] == answer_id][
                    "answer_content"
                ].iloc[0]
            results_df.loc[results_df["user_id"] == user_id, log.question_id] = (
                answer_content
            )
