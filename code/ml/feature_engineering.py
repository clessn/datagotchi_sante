...
def create_numerical_features(df_attributes_sandbox):
    numerical_fields = [
    ...
    return df_attributes_sandbox.loc[:, numerical_variables]
...
df_numerical_features = create_numerical_features(df_attributes_sandbox)
df_features = df_numerical_features  # aggregate here different type of features
df_targets = df_attributes_sandbox.loc[:, C.TARGET_COLS]
df_features.to_csv(C.ML_PATH / C.FEATURES_SANDBOX_FILENAME)
df_targets.to_csv(C.ML_PATH / C.TARGETS_SANDBOX_FILENAME)
