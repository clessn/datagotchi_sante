...
# Score de santé mentale émotionnelle (hédonique), varie de 0 à 3 (repose sur 3 variables)
data_score$sante_emotion <- data_clean$bien_etre_felt_happy + data_clean$bien_etre_interested_in_life + data_clean$bien_etre_satisfied_with_life

# Score de fonctionnement positif, varie de 0 à 11 (repose sur 11 variables)
data_score$fonction_pos <- data_clean$bien_etre_important_contribution_society + data_clean$bien_etre_sense_belonging_community + data_clean$bien_etre_society_becomes_better_place + data_clean$bien_etre_people_fundamentally_good + data_clean$bien_etre_society_makes_sense + data_clean$bien_etre_like_personality + data_clean$bien_etre_handling_responsibilities + data_clean$bien_etre_warm_relationships_with_people + data_clean$bien_etre_better_person + data_clean$bien_etre_able_express_opinions + data_clean$bien_etre_life_has_meaning

# Score total, varie de 0 à 14 (somme de santé mentale émotionnelle et de fonctionnement positif)
data_score$score_tot <- data_score$sante_emotion + data_score$fonction_pos

# Score normalisé, càd ramené entre 0 et 1
data_score$score_norm <- data_score$score_tot/14
...
data_score$flourishing <- if_else(
...
data_score$languishing  <- if_else(
...
data_score$moderately_mental <- if_else(
  (data_score$flourishing==0)
  & (data_score$languishing==0),
  1,0)